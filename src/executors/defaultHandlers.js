...
// claude 輸出進 NDJSON log 的截尾上限(保尾不保頭:結尾是結論/摘要,較有價值)。
...
//   ai_run    - 叫 claude 進真實專案、讀該專案的 SKILL.md 並執行(改本體,不 commit),輸出進 log
...
function make(ops) {
  ...
  return {
    ...
    async ai_run({ task, emit }) {
      ...
      const output = ops.runClaude(def.prompt(task), src);
    ...
    async summarize({ workDir, task, shared }) {
      ...
      // 也可能是 headless claude 自作主張;無論何者都如實回報,不誤判成「沒做事」。
